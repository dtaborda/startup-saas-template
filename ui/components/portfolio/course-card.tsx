...
export function CourseCard({ course, enrollment }: CourseCardProps) {
  ...
  return (
    <Card
      variant="glass"
      className="group relative cursor-pointer overflow-hidden rounded-[1.75rem] border-border/70 bg-background/65 transition-all duration-300 hover:-translate-y-1 hover:border-primary/35 hover:shadow-[var(--glow-primary-sm)]"
    >
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),transparent_26%)] opacity-80" />
      ...
      >
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.12),transparent_34%),linear-gradient(180deg,rgba(5,5,8,0.08),rgba(5,5,8,0.5))]" />
        <div className="absolute left-4 top-4 flex items-center gap-2">
          <Badge
            variant={levelStyle.variant}
            className="px-2.5 py-1 text-[0.62rem] uppercase tracking-[0.22em]"
          >
            {levelStyle.label}
          </Badge>
          <span
            className={cn(
              "inline-flex items-center rounded-full border px-2.5 py-1 font-mono text-[0.62rem] uppercase tracking-[0.22em]",
              accent,
            )}
          >
        ...
        <div className="relative flex flex-col items-center gap-4 text-center">
          <div className="flex size-20 items-center justify-center rounded-[1.5rem] border border-white/10 bg-background/45 shadow-[var(--glow-primary-sm)] backdrop-blur-xl">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 px-4">
            {course.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="rounded-full border border-white/10 bg-background/45 px-2.5 py-1 font-mono text-[0.62rem] uppercase tracking-[0.2em] text-foreground/70 backdrop-blur"
              >
                {tag}
              </span>
            ))}
      ...
      <CardContent className="relative flex flex-col gap-4 p-5">
        ...
        <div className="grid grid-cols-3 gap-3 rounded-[1.25rem] border border-border/60 bg-background/55 p-3 text-xs text-muted-foreground">
          <div className="flex flex-col gap-2">
        ...
        {isEnrolled ? (
          <div className="flex flex-col gap-2 rounded-[1.25rem] border border-border/60 bg-background/45 p-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          {course.tags.map((tag) => (
            <Badge
              key={tag}
              variant="outline"
              className="border-border/70 bg-background/55 px-2 py-0.5 text-[0.62rem] uppercase tracking-[0.18em] text-muted-foreground"
            >
              {tag}
            </Badge>
          ))}
