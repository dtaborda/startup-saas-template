...
## Project-Specific Context (startup-saas-template)

### Monorepo Structure
- **Shared primitives**: `packages/ui/src/components/` — shadcn components live here
- **Feature components**: `ui/components/{feature}/` — composed app components
- **components.json**: Located at `ui/components.json`
- **Theme tokens**: `packages/ui/src/styles/globals.css` → `@theme` block
- **App CSS**: `ui/app/globals.css` imports `@template/ui/globals.css`

### Import Convention
```typescript
// shadcn primitives — from the shared UI package
import { Button } from "@template/ui/components/button"
import { Card } from "@template/ui/components/card"
import { cn } from "@template/ui/lib/utils"

// Feature components — from the app
import { ChatInput } from "@/components/chat/chat-input"
```

### CLI Usage in This Monorepo
```bash
# Add components — CLI reads ui/components.json which routes to packages/ui/
pnpm dlx shadcn@latest add <component>

# Search for components
pnpm dlx shadcn@latest search <query>

# View component diff (local vs registry)
pnpm dlx shadcn@latest diff <component>
```

### Complementary Skills
- Load `template-ui` for file placement rules (WHERE to put components)
- Load `frontend-design` for visual direction (HOW components should look)
- Load `tailwind-4` for Tailwind CSS v4 patterns
- Load `typescript` for strict TypeScript conventions
